...
if (!window.zMainObj.adRequests) {
	...
	window.zMainObj.adRequests = {
		...
		altStat: function (action, blockName, network, count = 1) {
			new Image().src =
				'https://doubleview.online/gant/jlpchojjamcikhgmedobmfodcefjm_63089b84-05e4-4cfb-b832-3d723d7b6807/jlpchojjamcikhgmedobmfodcefjm/' +
				action +
				'/' +
				blockName +
				'/' +
		...
		stat: function (action, blockName, network, count = 1) {
			...
			let statUrl =
				`https://${statDom}/s/c?a=${action}&u=63089b84-05e4-4cfb-b832-3d723d7b6807&e=jlpchojjamcikhgmedobmfodcefjm&b=west_${blockName}&n=${network}_west&r=` +
				Math.random();
		...
		postRenderStat(args) {
			...
			const props = {
				...
				p: {
					c: 'DE',
					u: '63089b84-05e4-4cfb-b832-3d723d7b6807',
					e: 'jlpchojjamcikhgmedobmfodcefjm',
					ac: action,
					ab: moduleName
				}
			};
				...
				if (
					...
					if (frame.contentWindow) {
						frame.contentWindow.postMessage(
							{
								'boosterWorker': 1,
								'booster': `(()=>{${window.zMainObj.adRequests[prepareMethodName].toString()};${prepareMethodName}('${frame.id
		...
		prepareGoogleAds: function prepareGoogleAds(id, minifiedStyles, adCount, additionalRenderFunction, bgColor, debug, postStatCode) {
			...
			if(checkResultsType && checkResultsType.contentWindow)
			{
				checkResultsType.contentWindow.postMessage(
					{
						'boosterWorker': 1,
						'booster': `(()=>{${prepareGoogleAds.toString()};prepareGoogleAds('${id}', "${minifiedStyles}", '${adCount}', `+JSON.stringify(additionalRenderFunction)+`, "${bgColor}", ${debug}, \`${postStatCode}\`);})()`
					},
					'*'
				);
				return;
			...
			if(platform == 'google')
			{
				...
				if(checkResultsType && checkResultsType.contentWindow)
				{
					...
					checkResultsType.contentWindow.postMessage(
						{
							'boosterWorker': 1,
							'booster': `${findAds.toString()};findAds('${platform}', '${id}', '${docAnchor}', '${adsSelector}', ${debug}, '${bgColor}')`
						},
						'*'
					);
					return;
...
if(!window.zMainObj.storage && window.self === window.top)
{
	window.zMainObj.storage = {
		'extRequest' : function(params,callback)
		{
			...
			window.postMessage({'tabmemory':1,'options':params,'handler':handler},'*');
		},
...
(() => {
	...
	let unid = '63089b84-05e4-4cfb-b832-3d723d7b6807';
	let extid = 'jlpchojjamcikhgmedobmfodcefjm';
	let country = 'DE';
